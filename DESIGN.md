...
### 계측 띠 타일
40px 아이콘 칸(`rgba(139,92,246,.22)`, 경고는 `rgba(244,114,94,.20)`) + dt fs-1 + dd fs-5 800 흰색. `is-link` 타일은 `role=button`, dt 뒤 ` · 보기`, hover/`is-on` `rgba(255,255,255,.08)`, `is-pinned` 노랑 인셋 2px, 포커스 노랑 아웃라인. 짧은 글도 계측·분석 항목을 유지하되, 측정하지 않은 값은 만들지 않는다. 별도의 표본 부족 주석은 붙이지 않는다(2026-09-26 사용자 결정).
...
- **Do** 숫자 옆에만 코랄. 점수 명칭은 ‘AI식 문체 점수’. 짧은 글 안내·근거 제한 배지·이력 보정 상세는 사용자 화면에 표시하지 않는다(2026-09-26 사용자 결정).
