...
test('관리자 패치노트 탭은 운영 반영 이력을 최신순으로 제공한다', async () => {
  ...
  assert.equal(admin.match(/class="gp-admin-patch-release"/gu)?.length, 84);
  ...
  assert.match(admin, /휴머나이징 v2\.5\.67 · AI 감지 v1\.41/u);
  ...
  assert.equal(releases.length, 84);
  assert.equal(releases.filter(([, attrs]) => /\bopen\b/u.test(attrs)).length, 32);
  assert.match(releases[0][1], /\bopen\b/u);
  assert.match(releases[0][2], /PDF 추출 행·열과 공백 처리 무결성 보강/u);
