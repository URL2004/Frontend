...
test('관리자 패치노트 탭은 운영 반영 이력을 최신순으로 제공한다', async () => {
  ...
  assert.equal(admin.match(/class="gp-admin-patch-release"/gu)?.length, 89);
  ...
  assert.match(admin, /휴머나이징 v2\.5\.77 · AI 감지 v1\.44/u);
  ...
  assert.equal(releases.length, 89);
  assert.equal(releases.filter(([, attrs]) => /\bopen\b/u.test(attrs)).length, 35);
  assert.match(releases[0][1], /\bopen\b/u);
  assert.match(releases[0][2], /문단 연결 정보와 의견·가능성 보존/u);
