...
test('다듬기·기본·고급 명칭과 설명은 선택부터 결과·이력까지 같은 의미를 사용한다', async () => {
  ...
  assert.match(main, /맞춤법·연결만 정리/u);
...
test('진단 선택 섹션은 상태 라벨과 중복 유도 없이 핵심 정보만 보여준다', async () => {
  const [main, evasion] = await Promise.all([
    read('pages/main.html'),
    read('assets/js/evasion-flow.js')
  ]);
  assert.match(main, /id="lavChoiceSummary"[^>]*data-state="needs-info"/u);
...
test('모드 추천을 열고 3택 카드 문구는 엔진이 실제로 하는 일만 말한다', async () => {
  ...
  assert.doesNotMatch(main, /id="lavRecommendedMode"|id="lavRecommendedReason"|추천 시작점/u);
...
test('관리자 패치노트 탭은 운영 반영 이력을 최신순으로 제공한다', async () => {
  ...
  assert.equal(admin.match(/class="gp-admin-patch-release"/gu)?.length, 85);
  ...
  assert.match(admin, /휴머나이징 v2\.5\.68 · AI 감지 v1\.42/u);
  assert.match(admin, /2026\.06\.04 — 09\.25/u);
  ...
  assert.match(admin, /Backend 23a3624/u);
  ...
  assert.equal(releases.length, 85);
  assert.equal(releases.filter(([, attrs]) => /\bopen\b/u.test(attrs)).length, 33);
  assert.match(releases[0][1], /\bopen\b/u);
  assert.match(releases[0][2], /원문 대비 신규 오류와 장문 열거·복구 무결성 보강/u);
