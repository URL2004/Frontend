...
test('관리자 패치노트 탭은 운영 반영 이력을 최신순으로 제공한다', async () => {
  ...
  assert.equal(admin.match(/class="gp-admin-patch-release"/gu)?.length, 65);
  ...
  assert.match(admin, /휴머나이징 v2\.5\.51 · AI 감지 v1\.32/u);
  assert.match(admin, /2026\.06\.04 — 09\.14/u);
  ...
  assert.equal(releases.filter(([, attrs]) => /\bopen\b/u.test(attrs)).length, 24);
