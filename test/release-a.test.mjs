...
test('관리자 패치노트 탭은 운영 반영 이력을 최신순으로 제공한다', async () => {
  ...
  assert.equal(admin.match(/class="gp-admin-patch-release"/gu)?.length, 86);
  ...
  assert.equal(releases.length, 86);
  assert.equal(releases.filter(([, attrs]) => /\bopen\b/u.test(attrs)).length, 34);
  assert.match(releases[0][1], /\bopen\b/u);
  assert.match(releases[0][2], /관계 보존과 AI식 문체 점수 설명 개선/u);
