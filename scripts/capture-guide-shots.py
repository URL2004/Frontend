...
with sync_playwright() as p:
    ...
    page.evaluate(
        """(rows) => {
      window.lavFlowGo('report');
      const set = (id, v) => { const e = document.getElementById(id); if (e) e.textContent = v; };
      set('lavRepProb', '74');
      const badge = document.getElementById('lavRepBadge');
      badge.hidden = false; badge.textContent = 'AI 티 지수 높음'; badge.className = 'lav-rep-badge bad';
      document.getElementById('lavRepScore').className = 'lav-rep-hero bad';
      const arc = document.getElementById('lavRepArc');
      const LEN = Math.PI * 90;
      arc.style.strokeDasharray = LEN;
      arc.style.strokeDashoffset = LEN * (1 - 74 / 100);
      set('lavRepTitle', 'AI가 쓴 글처럼 보일 신호가 많아요');
      set('lavRepSummary', '상투적인 연결어와 균일한 문장 길이가 여러 문단에서 반복돼요.');
      set('lavRepStatRisk', '2'); set('lavRepStatThin', '1'); set('lavRepStatSafe', '1');
      const list = document.getElementById('lavRepParaList');
      list.innerHTML = '';
      rows.forEach(([kind, chip, text, why]) => {
        const row = document.createElement('div');
        row.className = 'lav-rep-para ' + kind;
        const c = document.createElement('span'); c.className = 'rp-chip'; c.textContent = chip;
        const body = document.createElement('div'); body.className = 'rp-body';
        const s = document.createElement('p'); s.textContent = text;
        const em = document.createElement('em'); em.textContent = why;
        body.appendChild(s); body.appendChild(em);
      ...
      if (cost) { cost.hidden = false; cost.textContent = '이동 후 기본 휴머나이징 10크레딧 · 보유 200크레딧'; }
    }""",
        PARAGRAPHS,
    )
    page.wait_for_timeout(600)
    shot(page, "result-detect", selector=".lav-report", pad=0)
    page.close()
